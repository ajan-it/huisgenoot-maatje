-- Create the integrity view first
CREATE OR REPLACE VIEW public.plan_integrity AS
WITH per_plan AS (
  SELECT
    p.id as plan_id,
    p.household_id,
...
-- Clean up ghost plans (plans with zero occurrences and zero selected tasks)
UPDATE public.plans 
SET status = 'draft'
WHERE id IN (
  SELECT plan_id 
  FROM public.plan_integrity 
  WHERE occurrences = 0 AND selected_active = 0
);